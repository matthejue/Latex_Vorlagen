...
\begin{frame}[fragile,allowframebreaks]{Network}
  ...
  \begin{Sidenote}
    \begin{itemize}
      \item \alert{other packages:} \inlinebox{pacman -S wpa_supplicant wireless_tools netctl}, if there's no wired connection one can use \inlinebox{wifi-menu} from the \inlinebox{netctl} package.
    \end{itemize}
  \end{Sidenote}
\end{frame}
